        ...
        if prev_streamer and prev_streamer.is_running():
            if width != prev_streamer.width or height != prev_streamer.height:
                ...
                try:
                    ...
                    # Wait for streamer to stop with a timeout
                    try:
                        await asyncio.wait_for(prev_streamer.wait(), timeout=5.0)
                        logging.info("Previous streamer stopped successfully")
                    except asyncio.TimeoutError:
                        logging.warning("Timeout waiting for previous streamer to stop, forcing cleanup")
                        # Force cleanup of the previous streamer
                        await prev_streamer._do_stop()
                except Exception as e:
                    logging.error(f"Error stopping previous streamer: {e}")
                    # Continue with new streamer even if old one failed to stop cleanly
                    # The old streamer's resources will be cleaned up by the supervisor task
