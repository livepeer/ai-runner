...
async def run_subscribe(subscribe_url: str, image_callback, put_metadata, monitoring_callback, output_width, output_height):
    ...
    try:
        ...
        parse_task = asyncio.create_task(decode_in(in_pipe, image_callback, put_metadata, write_fd, output_width, output_height))
...
async def decode_in(in_pipe, frame_callback, put_metadata, write_fd, output_width=512, output_height=512):
    def decode_runner():
        retry_count = 0
        last_retry_time = time.time()
        while retry_count < MAX_DECODER_RETRIES:
            try:
                decode_av(f"pipe:{in_pipe}", frame_callback, put_metadata, output_width, output_height)
                break  # clean exit
            except Exception as e:
                msg = str(e)
...
def encode_in(task_pipes, task_lock, image_generator, sync_callback, get_metadata, output_width, output_height, **kwargs):
    ...
    while retryCount < MAX_ENCODER_RETRIES:
        try:
            encode_av(image_generator, sync_callback, get_metadata, output_width=output_width, output_height=output_height, **kwargs)
            break  # clean exit
...
async def run_publish(publish_url: str, image_generator, get_metadata, monitoring_callback, output_width, output_height):
    ...
    try:
        ...
        encode_thread = threading.Thread(
            target=encode_in, 
            args=(live_pipes, live_tasks_lock, image_generator, sync_callback, get_metadata, output_width, output_height),
