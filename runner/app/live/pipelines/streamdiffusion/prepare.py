...
LOGGER = logging.getLogger("streamdiffusion.prepare")
...
def prepare_streamdiffusion_models(models_dir: Path) -> None:
    ...
    LOGGER.info("Preparing StreamDiffusion assets in %s", models_dir)
    _compile_dependencies(models_dir, engines_dir)
    jobs = list(_build_matrix())
    LOGGER.info("Compilation plan has %d build(s)", len(jobs))
    ...
    LOGGER.info("StreamDiffusion model preparation complete.")
...
def _build_depth_anything(models_dir: Path, engines_dir: Path) -> None:
    ...
    if engine_path.exists():
        LOGGER.info("Depth-Anything engine already present: %s", engine_path)
        return

    LOGGER.info("Building Depth-Anything TensorRT engine...")
    ...
    LOGGER.info("Depth-Anything engine written to %s", engine_path)
...
def _build_pose_engines(models_dir: Path, engines_dir: Path) -> None:
    ...
    for asset in POSE_ASSETS:
        ...
        if engine_path.exists():
            LOGGER.info("Pose engine already present: %s", engine_path)
            continue

        engine_path.parent.mkdir(parents=True, exist_ok=True)
        LOGGER.info("Building pose engine for %s", onnx_path.name)
        ...
        LOGGER.info("Pose engine written to %s", engine_path)
...
def _build_raft_engine(engines_dir: Path) -> None:
    ...
    if engine_path.exists():
        LOGGER.info("RAFT engine already present: %s", engine_path)
        return

    LOGGER.info("Compiling RAFT TensorRT engine...")
    engine_path.parent.mkdir(parents=True, exist_ok=True)
    subprocess.run(
        [
            sys.executable,
    ...
    LOGGER.info("RAFT engine written to %s", engine_path)
    ...
    for model_id, model_type in MODEL_ID_TO_TYPE.items():
        ...
        for ip_type in ip_types:
            ...
            if model_type not in control_keys:
                LOGGER.warning("Unknown controlnet set for model type %s", model_type)
            yield BuildJob(
                model_id=model_id,
