...
class AudioToTextPipeline(Pipeline):
    def __init__(self, model_id: str):
        ...
        self.backend = os.getenv("BACKEND", "faster-whisper")
        torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
        if self.backend == "faster-whisper":
            from faster_whisper import WhisperModel
            self.tm = WhisperModel(model_id, device=torch_device, compute_type=torch_dtype)
        else:
        ...
        if self.backend == "faster-whisper":
            if kwargs["return_timestamps"] == "word":
                raise InferenceError(
                    f"Word timestamps are not supported for model {self.model_id}."
                )
            segments, info = self.tm.transcribe("audio.mp3", 
                                                beam_size=5, 
